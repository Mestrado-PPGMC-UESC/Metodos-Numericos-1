...
double* lerVetorSolucaoExata(const char* nomeArquivo, int* ordemOut) {
    FILE* arquivo = fopen(nomeArquivo, "r");
    if (!arquivo) {
        fprintf(stderr, "Erro: não foi possível abrir o arquivo '%s'.\n", nomeArquivo);
        exit(1);
    ...
    int n;
    if (fscanf(arquivo, "%d", &n) != 1 || n <= 0) {
        fprintf(stderr, "Erro: primeira linha deve conter o tamanho do vetor solução.\n");
        fclose(arquivo);
        exit(1);
    }
    ...
    *ordemOut = n;
    ...
    for (int i = 0; i < n; i++) {
        if (fscanf(arquivo, "%lf", &vetor[i]) != 1) {
            fprintf(stderr, "Erro ao ler valor da solução exata no índice %d.\n", i);
            fclose(arquivo);
            exit(1);
        }
    }
...
void calcularErroRelativoGenerico(const double* xNum,
                                  const double* xExata,
                                  int ordemMatriz) {
    ...
    printf("\nErros relativos (em %%):\n");
    for (int i = 0; i < ordemMatriz; i++) {
        double erroPercentual = fabs(xNum[i] - xExata[i]) / fabs(xExata[i]) * 100.0;
        printf("A[%d] = %.6e %%\n", i, erroPercentual);
...
void processarSolucaoExata(const char* nomeArquivo,
                           const double* solucaoNumerica,
                           int ordemMatriz)
{
    int ordemSolExata = 0;

    double* solucaoExata =
        lerVetorSolucaoExata(nomeArquivo, &ordemSolExata);

    if (ordemSolExata != ordemMatriz) {
        fprintf(stderr,
            "[AVISO] Vetor solução exata tem dimensão %d, "
            "mas a matriz tem %d. Erro relativo não calculado.\n",
            ordemSolExata, ordemMatriz);
        free(solucaoExata);
        return;
    }

    printf("\n=== Comparação com solução exata de '%s' ===\n",
           nomeArquivo);

    calcularErroRelativoGenerico(solucaoNumerica,
                                 solucaoExata,
                                 ordemMatriz);
